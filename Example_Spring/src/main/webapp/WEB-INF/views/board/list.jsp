...
<script>
	...
	function list(page){
		location.href="/board/board/list.do?board_typeCode=${board_typeCode}&curPage="+page+"&searchOption=${map2.searchOption}"+"&keyword=${map2.keyword}";
	}
</script>
...
<div class="container">
	...
	<form name="form1" method="post" action="list.do">
		...
		<select name="searchOption">
			...
			<option value="board_writer" <c:out value="${map2.searchOption == 'board_writer'?'selected':''}"/> >작성자</option>
			<option value="board_title" <c:out value="${map2.searchOption == 'board_title'?'selected':''}"/> >제목</option>
			<option value="board_content" <c:out value="${map2.searchOption == 'board_content'?'selected':''}"/> >내용</option>
		</select>
		...
		<label><input type="radio" name="board_time" id="board_time" value="30" 
			<c:out value="${map2.board_time == '30'?'checked':''}"/>/>1달전</label>
		<label><input type="radio" name="board_time" id="board_time" value="365" 
			<c:out value="${map2.board_time == '365'?'checked':''}"/>/>1년전</label>
	...
	<thead>
		...
		<tr>
			<td colspan="5">
				...
				<c:if test="${map2.pagingDto.curBlock > 1}">
					<a href="javascript:list('${map2.pagingDto.prevPage}')">[이전]</a>
				</c:if>
				...
				<c:if test="${map2.pagingDto.curBlock <= map2.pagingDto.totBlock}">
					<a href="javascript:list('${map2.pagingDto.nextPage}')">[다음]</a>
				</c:if>
				
				<!-- 끝페이지로 이동 : 현재 페이지가 전체 페이지보다 작거나 같으면 [끝]하이퍼링크를 화면에 출력 -->
				<c:if test="${map2.pagingDto.curPage <= map2.pagingDto.totPage}">
					<a href="javascript:list('${map2.pagingDto.totPage}')">[끝]</a>
				</c:if>
			</td>
		</tr>
		<!-- 페이징 -->
